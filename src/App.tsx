...
import { Faq, Privacy, Safety, Terms } from './components/legal/LegalContent';
import { GameNav, SiteFooter } from './components/navigation/GameNav';
...
const wouldYouRatherRounds: WouldYouRatherRound[] = [
  {
    a: 'Slow kiss before every answer',
    b: 'One bold dare after every answer',
  },
  {
    a: 'A secret date planned by you',
    b: 'A surprise night planned by them',
  },
  {
    a: 'Only whispers for ten minutes',
    b: 'Only touch, no words, for ten minutes',
  },
  {
    a: 'Answer one spicy question honestly',
    b: 'Let your partner choose one soft dare',
  },
  {
    a: 'Replay your first-date energy',
    b: 'Invent a completely new first date',
  },
  {
    a: 'Keep the lights low',
    b: 'Let the playlist decide the mood',
  },
  {
    a: 'Trade compliments for one minute',
    b: 'Trade confessions for one minute',
  },
  {
    a: 'Be teased slowly',
    b: 'Be kissed without warning',
  },
  {
    a: 'Let them ask anything, once',
    b: 'Let them choose the next move, once',
  },
  {
    a: 'A soft rule for the whole round',
    b: 'A bold rule for only one minute',
  },
  {
    a: 'Say exactly what you want',
    b: 'Make them guess with three clues',
  },
  {
    a: 'Start with a compliment',
    b: 'Start with a confession',
  },
];
...
function pickNextWouldYouRatherIndex(currentIndex: number) {
  if (wouldYouRatherRounds.length === 1) {
    return 0;
  }

  let nextIndex = Math.floor(Math.random() * wouldYouRatherRounds.length);
  while (nextIndex === currentIndex) {
    nextIndex = Math.floor(Math.random() * wouldYouRatherRounds.length);
  }
  return nextIndex;
}
...
function ResultShareLink({ text }: { text: string }) {
  return (
    <a className="result-share-link" href={createWhatsappShareUrl(text)} target="_blank" rel="noreferrer">
...
export default function App() {
  ...
  const copyShareLink = useCallback(async () => {
    try {
      await navigator.clipboard.writeText(createShareMessage(SHARE_INVITE_TEXT));
  ...
  const shareInvite = useCallback(async () => {
    const shared = await shareText(BRAND_NAME, SHARE_INVITE_TEXT);
    if (shared) {
      setShareOpen(false);
    }
  }, []);
    ...
    return (
      <main className="page age-page">
        <div className="age-content">
          ...
          <SiteFooter onOpen={setLegalPanel} showGameNav={false} />
        </div>
      </main>
    );
  }
  ...
  if (currentPath === '/would-you-rather-couples/') {
    return <WouldYouRatherPage />;
  }

  if (currentPath === '/never-have-i-ever/') {
    return <NeverHaveIEverPage />;
  }
  ...
  if (currentPath === '/') {
    return <HomePage onOpenLegal={setLegalPanel} />;
  }

  if (currentPath !== '/bedroom-roulette/') {
    return <HomePage onOpenLegal={setLegalPanel} />;
  }
    ...
    <main className={['truth-dare-page', spinning ? 'is-spinning' : ''].filter(Boolean).join(' ')}>
      ...
      >
        <section className="td-card" aria-live="polite" aria-label="Selected Truth or Dare card">
          ...
          {activeCard && (
            <ResultShareLink
              text={`${activeCard.kind === 'dare' ? '🎯 Dare' : '💬 Truth'}: ${activeCard.text}`}
            />
          )}
        </section>
      </div>
...
const neverHaveIEverPrompts = [
  ...
  'sent a text I immediately regretted.',
  'kissed someone on the first date.',
  'stalked an ex on social media.',
  'fallen for someone way too fast.',
  'pretended to be busy to dodge a date.',
  'flirted just to make someone jealous.',
  'texted an ex after midnight.',
  "had a crush on a friend's partner.",
  'said “I love you” first.',
  'danced with a complete stranger.',
  'lied about my “type”.',
  'kept a photo I should have deleted.',
  'caught feelings from a single glance.',
  'gone on a date just for the free dinner.',
  'dreamt about someone in this room.',
  'sent a risky text and waited nervously.',
  "forgotten someone's name mid-flirt.",
  'pretended not to be interested when I was.',
  'swiped right out of pure boredom.',
];

function NeverHaveIEverPage() {
  const [pouring, setPouring] = useState(false);
  const [done, setDone] = useState(false);
  const [liquidFull, setLiquidFull] = useState(false);
  const [overlayOpen, setOverlayOpen] = useState(false);
  const [prompt, setPrompt] = useState('');
  const [hint, setHint] = useState('tap the bottle to pour');
  ...
  const pickPrompt = useCallback(() => {
    let index = Math.floor(Math.random() * neverHaveIEverPrompts.length);
    while (index === lastPromptRef.current && neverHaveIEverPrompts.length > 1) {
      index = Math.floor(Math.random() * neverHaveIEverPrompts.length);
    }
    lastPromptRef.current = index;
    return neverHaveIEverPrompts[index];
  }, []);

  const playGlug = useCallback(() => {
    try {
      const AudioContextClass = window.AudioContext || (window as WebAudioWindow).webkitAudioContext;
      if (!AudioContextClass) {
        return;
      ...
      for (let index = 0; index < 5; index += 1) {
        ...
        oscillator.frequency.setValueAtTime(150 + Math.random() * 60, now + index * 0.17);
        gain.gain.setValueAtTime(0.0001, now + index * 0.17);
        gain.gain.exponentialRampToValueAtTime(0.06, now + index * 0.17 + 0.02);
        gain.gain.exponentialRampToValueAtTime(0.0001, now + index * 0.17 + 0.14);
        oscillator.connect(gain).connect(audioContext.destination);
        oscillator.start(now + index * 0.17);
        oscillator.stop(now + index * 0.17 + 0.17);
      }

      window.setTimeout(() => void audioContext.close(), 1200);
    } catch {
      // Best-effort tap sound; interaction still works when Web Audio is blocked.
    }
  }, []);
  ...
  const pour = useCallback(() => {
    ...
    timersRef.current = [
      window.setTimeout(() => setLiquidFull(true), 320),
      window.setTimeout(() => {
        setPouring(false);
        setDone(true);
      }, 1350),
  ...
  }, [clearTimers, pickPrompt, playGlug, pouring]);

  const nextRound = useCallback(() => {
    setOverlayOpen(false);
    setHint('tap the bottle to pour');
    window.setTimeout(() => {
      setLiquidFull(false);
      setDone(false);
    }, 350);
  ...
  return (
    <main className={['never-page', pouring ? 'pouring' : ''].filter(Boolean).join(' ')}>
      <header className="nhie-header">
        <div className="nhie-crest" aria-hidden="true">
          ✦
        </div>
        <h1>
          Never Have
        ...
        <div className="nhie-sub">a game of confessions</div>
        <div className="nhie-rule" aria-hidden="true">
          <span />
          <i />
          <span />
        </div>
      ...
      <section
        className={['nhie-scene', pouring ? 'pour' : '', done ? 'done' : ''].filter(Boolean).join(' ')}
        id="scene"
        role="button"
        aria-label="Pour a glass"
        tabIndex={0}
        onClick={pour}
        onKeyDown={(event) => {
      ...
      >
        <div className="nhie-bottle-foot" />
        <svg className="nhie-bottle" viewBox="0 0 60 200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <defs>
            <linearGradient id="nhie-glass-b" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0" stopColor="#4a1520" />
              <stop offset="0.4" stopColor="#8c2f43" />
              <stop offset="0.54" stopColor="#a83b52" />
              <stop offset="0.7" stopColor="#7a2639" />
              <stop offset="1" stopColor="#3e1019" />
            </linearGradient>
            <linearGradient id="nhie-foil-b" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0" stopColor="#b98a54" />
              <stop offset="0.5" stopColor="#eccf96" />
              <stop offset="1" stopColor="#a67840" />
            </linearGradient>
          </defs>
          <path
            fill="url(#nhie-glass-b)"
            d="M 25 12 L 35 12 C 35 16 36 20 36 24 L 36 50 C 36 68 46 74 46 96 L 46 182 C 46 190 40 194 30 194 C 20 194 14 190 14 182 L 14 96 C 14 74 24 68 24 50 L 24 24 C 24 20 25 16 25 12 Z"
          />
          <rect x="24" y="2" width="12" height="8" rx="1.5" fill="url(#nhie-foil-b)" />
          <rect x="24" y="10" width="12" height="15" fill="url(#nhie-foil-b)" opacity="0.9" />
          <rect x="16.5" y="110" width="27" height="54" rx="2" fill="#f6efe2" />
          <rect x="16.5" y="110" width="27" height="54" rx="2" fill="none" stroke="#b08d4f" strokeWidth="0.8" strokeOpacity="0.65" />
          <text x="30" y="128" textAnchor="middle" fontFamily="Cormorant Garamond, serif" fontSize="6.5" fill="#b08d4f" letterSpacing="1.2">
            N.H.I.E
          </text>
          <text x="30" y="145" textAnchor="middle" fontFamily="Cormorant Garamond, serif" fontSize="10" fontStyle="italic" fill="#8c2f43">
            Rosé
          </text>
          <line x1="21" y1="153" x2="39" y2="153" stroke="#8a713f" strokeWidth="0.6" strokeOpacity="0.5" />
          <rect x="19" y="86" width="4" height="92" rx="2" fill="rgba(255,255,255,0.3)" />
        </svg>

        <div className="nhie-stream" />

        <svg className="nhie-shot" viewBox="0 0 60 80" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <defs>
            <clipPath id="nhie-shot-clip">
              <path d="M11 8 L49 8 L43 63 L17 63 Z" />
            </clipPath>
            <linearGradient id="nhie-shot-drink" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0" stopColor="#e79aad" />
              <stop offset="1" stopColor="#cf6a80" />
            </linearGradient>
          </defs>
          <path d="M11 8 L49 8 L43 63 L17 63 Z" fill="rgba(255,255,255,0.42)" />
          <g clipPath="url(#nhie-shot-clip)">
            <rect className="nhie-liquid" x="10" y="7" width="40" height="57" fill="url(#nhie-shot-drink)" style={{ transform: liquidFull ? 'scaleY(0.82)' : 'scaleY(0)' }} />
          </g>
          <path
            d="M17 63 L43 63 L45 74 C45 76.5 43.5 78 41 78 L19 78 C16.5 78 15 76.5 15 74 Z"
            fill="rgba(255,255,255,0.55)"
            stroke="#b08d4f"
            strokeOpacity="0.55"
            strokeWidth="1.3"
          />
          <path d="M17 63 L43 63" stroke="#b08d4f" strokeOpacity="0.4" strokeWidth="1" />
          <path d="M11 8 L49 8 L43 63 L17 63 Z" fill="none" stroke="#b08d4f" strokeOpacity="0.65" strokeWidth="1.5" />
          <line x1="13" y1="8" x2="47" y2="8" stroke="#ffffff" strokeOpacity="0.7" strokeWidth="1.2" />
          <rect x="16" y="13" width="2.6" height="46" rx="1.3" fill="rgba(255,255,255,0.5)" />
          <line x1="27" y1="10" x2="25.5" y2="61" stroke="#b08d4f" strokeOpacity="0.16" strokeWidth="0.7" />
          <line x1="33" y1="10" x2="34.5" y2="61" stroke="#b08d4f" strokeOpacity="0.16" strokeWidth="0.7" />
        </svg>

        <div className="nhie-glass-foot" />
      </section>

      <p className={hint === 'cheers' ? 'nhie-hint wine' : 'nhie-hint'}>{hint}</p>

      <GameNav current="never" />

      <div
        className={overlayOpen ? 'nhie-overlay show' : 'nhie-overlay'}
        onClick={(event) => {
          if (event.target === event.currentTarget) {
            nextRound();
          }
      ...
      >
        <section className="nhie-card">
          <div className="nhie-card-kicker">Never have I ever…</div>
          <div className="nhie-card-rule" aria-hidden="true">
            <span />
            <i />
            <span />
          </div>
          <p className="nhie-card-text">{prompt}</p>
          <div className="nhie-card-note">if you have — take a sip</div>
          <button className="nhie-card-close" onClick={nextRound}>
            next shot
          </button>
          {prompt && <ResultShareLink text={`😏 Never have I ever ${prompt}`} />}
        </section>
      </div>
    </main>
  );
  ...
  return (
    <main className={['truth-dare-page dice-page', rollingStep ? 'is-rolling' : ''].filter(Boolean).join(' ')}>
      ...
      {roll?.action && roll.target && roll.rule && (
        <div className="dice-result-wrap" aria-live="polite">
          <p className="dice-result">
            {roll.action} + {roll.target} + {roll.rule}
          </p>
          <ResultShareLink text={`😏 Lovers' Dice: ${roll.action} + ${roll.target} + ${roll.rule}`} />
        </div>
      )}
...
function WouldYouRatherPage() {
  ...
  const round = wouldYouRatherRounds[roundIndex];

  const dealNext = useCallback(() => {
    setSelectedChoice(null);
    setRoundIndex((currentIndex) => pickNextWouldYouRatherIndex(currentIndex));
    setDealing(false);
  ...
  const hint = selectedChoice ? 'now tell them why' : 'choose honestly, then explain why';

  return (
    <main className={['truth-dare-page dice-page wyr-page', dealing ? 'is-dealing' : ''].filter(Boolean).join(' ')}>
      <header className="td-header dice-header wyr-header">
        <h1>
          Would <em>You</em> Rather
        </h1>
        <div className="td-rule" />
      </header>

      <section className="wyr-stage" aria-label="Would You Rather game">
        <div className="wyr-shadow" aria-hidden="true" />
        <div className="wyr-board">
          <button
            className={selectedChoice === 'a' ? 'wyr-choice option-a selected' : 'wyr-choice option-a'}
            onClick={() => setSelectedChoice('a')}
            type="button"
            aria-pressed={selectedChoice === 'a'}
          >
            <span className="wyr-choice-label">Choice A</span>
            <span className="wyr-choice-text">{round.a}</span>
          </button>

          <div className="wyr-center" aria-hidden="true">
            <span className="wyr-center-line" />
            <span className="wyr-center-mark">or</span>
          </div>

          <button
            className={selectedChoice === 'b' ? 'wyr-choice option-b selected' : 'wyr-choice option-b'}
            onClick={() => setSelectedChoice('b')}
            type="button"
            aria-pressed={selectedChoice === 'b'}
          >
            <span className="wyr-choice-label">Choice B</span>
            <span className="wyr-choice-text">{round.b}</span>
          </button>
        </div>
      </section>

      <p className={selectedChoice ? 'td-hint wine' : 'td-hint'}>{hint}</p>

      <button className="td-roll-button" onClick={dealNext}>
        New Choice
      </button>

      {selectedChoice && (
        <ResultShareLink text={`🤔 Would you rather ${round.a} OR ${round.b}?`} />
      )}

      <GameNav current="rather" />
    </main>
  );
}
